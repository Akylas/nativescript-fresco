...
export function initialize(
  ...
  if (application.android) {
    if (config && config.isDownsampleEnabled) {
      let imagePipelineConfig = com.facebook.imagepipeline.core.ImagePipelineConfig.newBuilder(
...
export class FrescoDrawee extends commonModule.FrescoDrawee {
  ...
  public createNativeView() {
    this._android = new com.facebook.drawee.view.SimpleDraweeView(
      this._context
    );
    return this._android;
  }
      ...
      if (this.imageUri) {
        ...
        if (utils.isFileOrResourcePath(this.imageUri)) {
          ...
          if (this.imageUri.indexOf(utils.RESOURCE_PREFIX) === 0) {
            ...
            let identifier = res.getIdentifier(
              resName,
              "drawable",
        ...
        const progressiveRenderingEnabledValue =
          this.progressiveRenderingEnabled !== undefined
            ? this.progressiveRenderingEnabled
            : false;
        const requestBuilder = com.facebook.imagepipeline.request.ImageRequestBuilder.newBuilderWithSource(
          uri
        ...
        let listener = new com.facebook.drawee.controller.ControllerListener<
        ...
        >({
          ...
          onSubmit: function(id, callerContext) {
            if (that && that.get()) {
              ...
              console.log(
                "Warning: WeakRef<FrescoDrawee> was GC and no 'submitEvent' callback will be raised."
              );
            }
          }
        });
  ...
  private updateHierarchy() {
    if (this._android) {
      ...
      if (
        ...
        let bottomRightRadius = this.roundBottomRight
          ? this.roundedCornerRadius
          : 0;
        let bottomLeftRadius = this.roundBottomLeft
          ? this.roundedCornerRadius
          : 0;
  ...
  private getDrawableFromLocalFile(localFilePath: string) {
    ...
    if (img) {
      drawable = new android.graphics.drawable.BitmapDrawable(
        utils.ad.getApplicationContext().getResources(),
        img.android
      );
    }
  ...
  private getDrawableFromResource(resourceName: string) {
    let img = imageSource.fromResource(
      resourceName.substr(utils.RESOURCE_PREFIX.length)
    );
    let drawable: android.graphics.drawable.BitmapDrawable = null;
    if (img) {
      drawable = new android.graphics.drawable.BitmapDrawable(
...
class GenericDraweeHierarchyBuilder {
  ...
  constructor() {
    ...
    this.nativeBuilder = new com.facebook.drawee.generic.GenericDraweeHierarchyBuilder(
      res
    );
  }
  ...
  public setActualImageScaleType(
    scaleType: string
  ): GenericDraweeHierarchyBuilder {
    if (!application.android) {
      return null;
